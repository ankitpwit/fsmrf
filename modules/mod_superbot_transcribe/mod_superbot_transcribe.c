/* 
 *
 * mod_superbot_transcribe.c -- Freeswitch module for real-time transcription using superbot's gRPC interface
 *
 */
#include "mod_superbot_transcribe.h"
#include "superbot_glue.h"
#include <stdlib.h>
#include <switch.h>
...
SWITCH_MODULE_DEFINITION(mod_superbot_transcribe, mod_transcribe_load, mod_transcribe_shutdown, NULL);
...
static void responseHandler(switch_core_session_t* session, const char * json, const char* bugname, 
	...
	if (0 == strcmp("vad_detected", json)) {
		...
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "transcription-vendor", "superbot");
	}
...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
	...
	case SWITCH_ABC_TYPE_CLOSE:
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Got SWITCH_ABC_TYPE_CLOSE, calling superbot_speech_session_cleanup.\n");
			superbot_speech_session_cleanup(session, 1, bug);
	...
	case SWITCH_ABC_TYPE_READ:
		...
		return superbot_speech_frame(bug, user_data);
		break;
...
static switch_status_t do_stop(switch_core_session_t *session, char *bugname)
{
	...
	if (bug) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Received user command command, calling superbot_speech_session_cleanup (possibly to stop prev transcribe)\n");
		status = superbot_speech_session_cleanup(session, 0, bug);
...
static switch_status_t start_capture(switch_core_session_t *session, switch_media_bug_flag_t flags, 
  char* lang, char* dur_type, char* request_id, char* silence_duration, char* bugname)
{
	switch_channel_t *channel = switch_core_session_get_channel(session);
  ...
  if (!var) {
	...
	if (SWITCH_STATUS_FALSE == superbot_speech_session_init(session, responseHandler, samples_per_second, flags & SMBF_STEREO ? 2 : 1, lang, dur_type, request_id, silence_duration, bugname, &pUserData)) {
...
/*
  ...
  Macro expands to: switch_status_t mod_superbot_transcribe_shutdown() */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_transcribe_shutdown)
{
	superbot_speech_cleanup();
